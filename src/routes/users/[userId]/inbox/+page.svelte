  ...
  <section class="inbox-only-main">
    ...
    <div class="content">
      <div class="inbox-head">
        ...
        <span class="sync-status mobile-sync" class:syncing={autoRefreshing}>{autoRefreshLabel}</span>
      </div>
...
{:else}
  <div class="layout-shell">
    ...
    <section class="main">
      <AppTopbar
        title={`${$t('inbox.title')} - ${data.currentUser?.displayName ?? data.userId}`}
        breadcrumb="flash mail flare / pengguna / kotak masuk"
        bind:searchQuery
        searchPlaceholder={$t('inbox.search')}
      >
        <span slot="actions" class="sync-status" class:syncing={autoRefreshing}>{autoRefreshLabel}</span>
      ...
      <div class="content">
        <div class="inbox-head dashboard-head">
          <div class="title-wrap">
            <h1>{$t('inbox.title')}</h1>
            <span class="badge">{unreadCount} {$t('common.new')}</span>
          </div>
